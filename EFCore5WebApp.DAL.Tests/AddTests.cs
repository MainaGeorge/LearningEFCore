...
namespace EFCore5WebApp.DAL.Tests
{
    ...
    public class AddTests
    {
        ...
        public void InsertPersonWithAddresses()
        {
            ...
            var addedPerson = _context.Persons.Single(X => X.FirstName ==
                "Clarke" && X.LastName == "Kent");
            ...
            for (var i = 0; i < record.Addresses.Count; i++)
            {
                Assert.AreEqual(record.Addresses[i].AddressLine1,
                    addedPerson.Addresses[i].AddressLine1);
                Assert.AreEqual(record.Addresses[i].AddressLine2,
                    addedPerson.Addresses[i].AddressLine2);
                Assert.AreEqual(record.Addresses[i].City, addedPerson.
                    Addresses[i].City);
                Assert.AreEqual(record.Addresses[i].State, addedPerson.
                    Addresses[i].State);
                Assert.AreEqual(record.Addresses[i].ZipCode, addedPerson.
                    Addresses[i].ZipCode);
                Assert.AreEqual(record.Addresses[i].Country, addedPerson.
                    Addresses[i].Country);
            }
        }
        ...
        {
            var person = _context.Persons.Single(X => X.FirstName ==
                "Clarke" && X.LastName == "Kent");
